...
- 目标和设置保存在浏览器 `localStorage`；访问口令与监控开关保存在当前标签页的 `sessionStorage`，刷新页面会自动恢复监控。
- 支持浏览器通知、提示音、可选 Bark 服务端推送和有货时打开 Apple 页面。
- 通知按渠道记录成功状态；发送失败会在后续确认有货时重试，未知状态不重置已发送记录，明确无货后再次有货才重新提醒。记录保留在当前页面内，刷新后重新开始。
- 查询期间仍可调整设置、增删目标和打开商品页；暂停会取消当前查询。失败时显示本轮未知，并保留上次确认库存供参考。
...
云函数通过当前实例的内存限制同一客户端 5 秒内最多一轮；会话和冷却也缓存在当前实例。冷启动、多实例或不同区域之间不共享这些状态，因此这不是全局限流保证。若需要强制跨实例配额，应在部署平台配置网关限流，或接入支持原子操作的共享存储。EdgeOne 出口仍可能被 Apple 返回 541；界面会明确显示为「未知」。
